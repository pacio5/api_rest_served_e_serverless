...
app.post('/login', (req, res) => {
  ...
  UserModel.findOne({ _id: req.body.email }, (err, userFind) => {
    ...
    res.status(200).json({ message: 'Accesso eseguito', token });
  });
});
 ...
 * HTTP/1.1 200 OK
 * {
 *    message: Grafico salvato correttamente + url
 * }
 * 
...
app.post('/chart/save/', async (req, res) => {
  ...
  try {
    ...
    s3.upload(params, function (err, data) {
      ...
      res.status(200).json(`Grafico salvato correttamente. ${data.Location}`);
    });
...
// Applicazione in ascolto sulla porta desiderata
